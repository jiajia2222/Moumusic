...
export const getNewComment = async(musicInfo: LX.Music.MusicInfoOnline, page: number, limit: number, retryNum = 0): Promise<CommentInfo> => {
  let resp
  try {
    const comment = music[musicInfo.source]?.comment
    if (!comment) throw new Error('comments are not supported by the active user source')
    resp = await (comment.getComment(toOldMusicInfo(musicInfo), page, limit) as Promise<CommentInfo>)
  } catch (error: any) {
...
export const getHotComment = async(musicInfo: LX.Music.MusicInfoOnline, page: number, limit: number, retryNum = 0): Promise<CommentInfo> => {
  ...
  try {
    const comment = music[musicInfo.source]?.comment
    if (!comment) throw new Error('comments are not supported by the active user source')
    resp = await (comment.getHotComment(toOldMusicInfo(musicInfo), page, limit) as Promise<CommentInfo>)
  } catch (error: any) {
    console.log(error.message)
    if (error.message == '取消请求' || ++retryNum > 2) throw error
